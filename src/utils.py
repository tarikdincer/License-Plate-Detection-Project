...
def nms(Labels,iou_threshold=.5):
	...
	for label in label_list:
		...
		for sel_label in SelectedLabels:
			if IOU_labels(label[0],sel_label[0]) > iou_threshold:
				for label_temp in sel_label:
					if chr(label_temp.cl()) == chr(label[0].cl()):
						exist_in_list = True
				if exist_in_list == False:
					sel_label.append(label[0])
...
def nms_n(Labels,iou_threshold=.5):
